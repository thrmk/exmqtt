...
def table(devices):
    ...
    #reading=reading+1
    table_header=[
        html.Thead(html.Tr([html.Th('devId'),html.Th('sun angle') ,html.Th('tracker angle')#, html.Th('motor status') ,
         ]))]
    table_body=[
        html.Tbody(html.Tr([html.Td(dev.devId),html.Td(dev.SPA),html.Td(dev.TA)]))for dev in devices]
...
#print("In between")
app.layout = html.Div([navbar,content,data1,buttons,select,select2,dcc.Location(id="url",refresh=True)])
#@app1.route('/')
#def layout():
#    global x
"""
...
@app.callback(
    ...
    [dash.dependencies.Input('demo-dropdown', 'value1')],
